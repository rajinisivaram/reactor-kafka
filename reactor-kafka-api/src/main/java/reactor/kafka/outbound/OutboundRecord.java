 ...
 * {@link OutboundResponse} to its corresponding record.
 *
 * @param <K> Key type
 * @param <V> Value type
 */
public class OutboundRecord<K, V, T> {
     ...
     *        included in the response to match {@link OutboundResponse} to this record.
     * @return
     */
    public static <K, V, T> OutboundRecord<K, V, T> create(ProducerRecord<K, V> record, T correlationMetadata) {
        return new OutboundRecord<K, V, T>(record, correlationMetadata);
    }

    private OutboundRecord(ProducerRecord<K, V> record, T correlationMetadata) {
        this.record = record;
